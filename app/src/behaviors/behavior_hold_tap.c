...
struct behavior_hold_tap_config {
    ...
    enum flavor flavor;
    bool retro_tap;
    struct zmk_behavior_binding *retro_tap_behavior;
...
static int press_binding(struct active_hold_tap *hold_tap) {
    ...
    if (hold_tap->status == STATUS_HOLD_TIMER || hold_tap->status == STATUS_HOLD_INTERRUPT) {
        ...
        binding = *hold_tap->config->retro_tap_behavior;
...
static void decide_retro_tap(struct active_hold_tap *hold_tap) {
    ...
    if (hold_tap->status == STATUS_HOLD_TIMER) {
        ...
        if (!hold_tap->config->retro_tap_behavior) {
            hold_tap->status = STATUS_TAP;
        } else {
...
#define KP_INST(n)                                                                                 \
    static struct behavior_hold_tap_config behavior_hold_tap_config_##n = {                        \
        ...
        .retro_tap_behavior = DT_INST_PROP_OR(n, retro_tap_behavior, NULL),                        \
